          ...
          <div className="lg:col-span-2 space-y-4">
            <AnimatePresence>
              {items.map((item) => {
                ...
                return (
                  <motion.div key={item._id || `${pid}-${variantId}`} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {vname && (
                        <span className="text-xs bg-forest-50 text-forest-700 px-2 py-0.5 rounded-md font-bold mt-1 inline-block">
                          Variant: {vname}
                        </span>
                      )}
                    ...
                    <div className="flex flex-col items-end justify-between">
                      <button onClick={() => removeFromCart(pid, variantId)} className="text-gray-400 hover:text-red-500 transition-colors p-1">
                        <FiTrash2 className="text-sm" />
                      </button>
                      <div className="flex items-center border border-gray-200 rounded-xl overflow-hidden">
                        <button onClick={() => updateQuantity(pid, item.quantity - 1, variantId)} className="px-2.5 py-1.5 hover:bg-gray-100 transition-colors"><FiMinus className="text-xs" /></button>
                        <span className="px-2.5 py-1.5 text-sm font-semibold min-w-[30px] text-center">{item.quantity}</span>
                        <button onClick={() => updateQuantity(pid, item.quantity + 1, variantId)} className="px-2.5 py-1.5 hover:bg-gray-100 transition-colors"><FiPlus className="text-xs" /></button>
                      </div>
